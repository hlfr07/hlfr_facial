...
export class FacialRecognitionService {
    ...
    private async initializeModels() {
        try {
            ...
            const scrfdPath = path.join(process.cwd(), 'src', 'models', 'scrfd_10g_320_batch64.onnx');
    ...
    private async detectFace(imagePath: string): Promise<{
        ...
        try {
            ...
            // Detectar si los outputs tienen nombres (score_*, bbox_*) o índices numéricos
            const hasNamedOutputs = outputNames.some(name => name.includes('score') && name.includes('_'));

            let scoreKeys: string[] = [];
            let bboxKeys: string[] = [];

            if (hasNamedOutputs) {
                // Modelo 10g con outputs nombrados: score_8, bbox_8, score_16, bbox_16, etc.
                scoreKeys = featStrides.map(stride => `score_${stride}`).filter(k => results[k]);
                bboxKeys = featStrides.map(stride => `bbox_${stride}`).filter(k => results[k]);
            } else {
                // Modelo 2.5g con outputs numéricos
                ...
                scoreKeys = scoreIndices.map(i => outputNames[i]);
                bboxKeys = bboxIndices.map(i => outputNames[i]);
            }

            this.logger.log(`\n  Score keys: ${scoreKeys.join(', ')}`);
            this.logger.log(`  BBox keys: ${bboxKeys.join(', ')}`);

            if (scoreKeys.length === 0 || bboxKeys.length === 0) {
            ...
            for (let idx = 0; idx < scoreKeys.length; idx++) {
                ...
                const bboxKey = bboxKeys[idx];

                const scoresTensor = results[scoreKey];
                const bboxTensor = results[bboxKey];

                this.logger.log(`\n  📊 Processing scale ${idx + 1}:`);
                this.logger.log(`    Score tensor (${scoreKey}): dims=${JSON.stringify(scoresTensor.dims)}`);
                this.logger.log(`    BBox tensor (${bboxKey}): dims=${JSON.stringify(bboxTensor.dims)}`);

                // Extraer datos (puede tener batch dimension o no)
                let scores = scoresTensor.data as Float32Array;
                let bboxRaw = bboxTensor.data as Float32Array;

                this.logger.log(`    Raw data lengths: scores=${scores.length}, bbox=${bboxRaw.length}`);

                // Calcular stride desde el nombre de la clave o desde el número de elementos
                let stride = 8;
                if (scoreKey.includes('_16')) stride = 16;
                else if (scoreKey.includes('_32')) stride = 32;

                // Detectar esperado vs actual
                const gridSize = 320 / stride;
                const expectedScores = gridSize * gridSize * 2; // 2 anchors
                const expectedBbox = expectedScores * 4;

                this.logger.log(`    Grid ${gridSize}x${gridSize} stride=${stride}: expecting ${expectedScores} scores, ${expectedBbox} bbox`);

                // Si el tamaño de bbox es 4x el esperado, tenemos batch dimension
                if (bboxRaw.length === expectedBbox * 4 && scores.length === expectedScores * 4) {
                    this.logger.warn(`    ⚠️  Detected 4x oversizing (batch dimension?), skipping first dimension`);
                    // Extrae solo la primera "batches" worth si hay múltiples
                    const newScores = new Float32Array(expectedScores);
                    const newBbox = new Float32Array(expectedBbox);
                    newScores.set(scores.subarray(0, expectedScores));
                    newBbox.set(bboxRaw.subarray(0, expectedBbox));
                    scores = newScores;
                    bboxRaw = newBbox;
                } else if (bboxRaw.length !== expectedBbox || scores.length !== expectedScores) {
                    this.logger.warn(`    ⚠️  Scale mismatch: scores ${scores.length} vs ${expectedScores}, bbox ${bboxRaw.length} vs ${expectedBbox}`);
                    continue;
                }

                this.logger.log(`\n  📍 Scale ${idx} (${scoreKey}, stride=${stride}):`);
                ...
                this.logger.log(`    BBoxPreds created: ${bboxPreds.length} boxes (from ${bboxRaw.length} raw elements)`);
                this.logger.log(`    Centers: ${centers.length}, BBoxPreds: ${bboxPreds.length} - must match!`);

                // Verificar que los tamaños coincidan
                if (centers.length !== bboxPreds.length) {
                    this.logger.error(`    ❌ MISMATCH: centers=${centers.length} vs bboxPreds=${bboxPreds.length}`);
                    this.logger.error(`    ❌ This will cause errors in distance2bbox filtering`);
                    continue;
                }
