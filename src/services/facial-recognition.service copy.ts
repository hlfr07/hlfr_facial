...
export class FacialRecognitionService {
    ...
    private async initializeModels() {
        try {
            ...
            const arcfacePath = path.join(process.cwd(), 'src', 'models', 'arcface_w600k_r50_batch64.onnx');
    ...
    private async generateEmbedding(imagePath: string, box?: any): Promise<ProcessingResult> {
        ...
        try {
            ...
            for (let i = 0; i < 112 * 112; i++) {
                float32Data[i] = (data[i * 3] - 127.5) / 128.0;
                float32Data[112 * 112 + i] = (data[i * 3 + 1] - 127.5) / 128.0;
                float32Data[112 * 112 * 2 + i] = (data[i * 3 + 2] - 127.5) / 128.0;
            }
