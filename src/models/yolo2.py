...
MODEL_PATH = "yolo11n-seg-coco.onnx"  # tu modelo ONNX
...
def process_masks(proto_masks: np.ndarray, bboxes: list[BoundingBox], img_size: tuple) -> list[BoundingBox]:
    """
    Aplica las máscaras a cada BoundingBox, correctamente alineadas a la imagen original.
    """
    # proto_masks original: (1, 32, H, W)
    mask_width, mask_height = proto_masks.shape[2:]
    proto_masks = proto_masks.reshape(32, -1)  # (32, H*W)
    
    width, height = img_size

    # Itera sobre las cajas; extrae y aplica máscara; umbraliza máscara
    for box in bboxes:
